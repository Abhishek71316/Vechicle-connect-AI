# RoadGuardian AI - Gemini AI Router
# Standalone API endpoints for Gemini AI processing using HTTP (avoids protobuf conflicts)

from fastapi import APIRouter, HTTPException
import base64
import cv2
import numpy as np
from ..ai.gemini_http_service import get_gemini_http_service
from ..ai.driver_monitor import DriverMonitor

router = APIRouter(prefix="/api/ai", tags=["ai-gemini"])

# Initialize driver monitor instance
driver_monitor = DriverMonitor()

@router.post("/analyze-frame")
async def analyze_frame(frame_data: dict):
    """
    Analyze a single frame for driver state using dlib 68-landmark model
    """
    try:
        ...
        return {
            ...
            "data": driver_monitor.get_default_state()
        }
...
async def gemini_status():
    ...
    """
    try:
        service = get_gemini_http_service()
        if not service:
            return {
...
async def gemini_chat(request: dict):
    ...
    try:
        ...
        response = await service.chat(message, conversation_history)
...
async def analyze_accident(request: dict):
    ...
    try:
        ...
        analysis = await service.analyze_accident(accident_data)
...
async def analyze_iot(request: dict):
    ...
    try:
        ...
        explanation = await service.analyze_iot_data(sensor_data)
...
async def generate_report(request: dict):
    ...
    try:
        ...
        report = await service.generate_accident_report(incident_data)
...
async def analyze_image(request: dict):
    ...
    try:
        ...
        analysis = await service.analyze_image(image_data, context)
...
async def emergency_summary(request: dict):
    ...
    try:
        ...
        summary = await service.generate_emergency_summary(incident_data)
